    ...
    final class ViewModel: ObservableObject {
        ...
        func deleteSelectedResponse() {
            if let selectedResponse = responseToDelete {
                if selectedResponse.hasAudioReflection, let aBrain = audioBrain {
                    Task {@MainActor in
                        do {
                            try await aBrain.deleteAudioReflection(for: selectedResponse)
                            ...
                            NSLog(">>> Specific error: \(aBrain.errorMessage)")
